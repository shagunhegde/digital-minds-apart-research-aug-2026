...
def main() -> None:
    ...
    fig2 = plots.order_contrast(per_order, differences,
                                title=f"Order contrast (layer {op_layer}, k={args.k})")
    ...
    fig3 = plots.f2_sensitivity(KS, f2_obs, null_lo, null_hi, null_med,
                                title=f"f₂ sensitivity to k (layer {op_layer})")
    ...
    results = {
        "operating_point": {"layer": op_layer, "strength": meta["strength"],
    ...
    lines = [
        ...
        "",
        f"Operating point: layer {op_layer}, alpha_rel {meta['strength']}, "
