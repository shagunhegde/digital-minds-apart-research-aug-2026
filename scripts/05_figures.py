...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "artifacts" / "figures")
    args = ap.parse_args()
    ...
    fig1 = plots.cascade_bar(headline["f1"], headline["f2"], headline["f3"],
                             ...
                             title=f"Where the injected concept is lost "
                                   f"(inject L{op_layer}, read L{readout_primary}, k={args.k})")
    ...
    fig2 = plots.order_contrast(per_order, differences,
                                title=f"Order contrast (inject L{op_layer}, read L{readout_primary}, k={args.k})")
    ...
    results = {
        "operating_point": {"layer": op_layer, "readout_layer": readout_primary,
    ...
    lines = [
        ...
        "",
        f"Operating point: inject L{op_layer}, read L{readout_primary}, "
