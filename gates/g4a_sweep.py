...
def main() -> None:
    ...
    order_of = condition_of = layer_of = strength_of = None
    ...
    for name, shard in loaded.items():
        ...
        if not np.array_equal(shard["cell_order"], order_of) or \
           ...
           not np.array_equal(shard["cell_layer"], layer_of) or \
    ...
    w(f"  layers x strengths       {manifest['layers']} x {manifest['strengths']}")
    ...
    w(f"  sweep task               {manifest['task']!r}")
    for order, meta in manifest["prompt_meta"].items():
        w(f"  {order:<18} seq={meta['seq_len']} window={meta['window']}"
          f" answer={meta['clean_task_answer']!r}")
    ...
    w("\n  missing-cell map, by (layer, strength, order, condition)")
    if missing:
        w(f"    every cell of {len(missing)} concept(s) absent: {missing[:12]}")
    if order_of is not None:
        counts = Counter(
            (int(l), float(s), str(o), str(c))
            for l, s, o, c in zip(layer_of, strength_of, order_arr, cond_arr))
        w(f"    distinct cell coordinates          {len(counts)}")
        w(f"    coordinates appearing more than once "
          f"{sum(1 for v in counts.values() if v > 1)}")
        w(f"    expected distinct                  "
          f"{len(manifest['layers']) * len(manifest['strengths']) * len(manifest['orders']) * len(manifest['conditions'])}")
    ...
    w("  detection channel sanity, by condition (pooled over completed shards)")
    if order_of is not None and loaded:
        w(f"    {'condition':<16}{'order':<18}{'median P(true)':>15}{'IQR':>10}{'n':>8}")
        for condition in manifest["conditions"]:
            for order in manifest["orders"]:
                mask = (cond_arr == condition) & (order_arr == order)
                vals = np.concatenate([
                    s["p_true"][mask] for s in loaded.values()
                    if s["p_true"].shape[0] == expected_cells]) if mask.any() else np.array([])
                if vals.size:
                    d = stats.median_iqr(vals)
                    w(f"    {condition:<16}{order:<18}{d['median']:>15.4f}"
                      f"{d['iqr']:>10.4f}{vals.size:>8}")
    ...
    anomalies.append(
        "control_zero cells are computed once per (batch, layer, order) and "
        "broadcast across strengths. This is exact -- alpha=0 adds exactly zero, "
        "which G2 measures as the zero-strength identity -- but it means the "
        "condition carries no per-concept variation, so f1's control set should "
        "come from control_random, which is matched-norm and does vary.")
    anomalies.append(
        "the sweep uses a single fixed task, so there is no task-level "
        "variation in any channel; every interval here is over concepts and "
        "cells, not over tasks.")
    ...
    if order_of is not None:
        ...
        distinct_zero = len(manifest["layers"]) * len(manifest["orders"])
        per_batch = (expected_cells - n_zero_cells) + distinct_zero
        n_batches = int(np.ceil(len(concepts) / manifest["batch"]))
